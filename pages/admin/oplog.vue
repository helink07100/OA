  ...
  <LayoutPage title="操作日志">
    ...
    >
      ...
      >
        ...
        <el-table-column prop="do_ip" label="操作IP"> </el-table-column>
        <el-table-column prop="do_time" label="操作时间">
          <template slot-scope="scope">
            {{
              $moment(new Date(scope.row.do_time * 1000)).format(
                'YYYY-MM-DD HH:mm:ss'
              )
            }}
          </template>
        </el-table-column>
        <el-table-column prop="msg" width="200" label="操作说明">
          <template slot-scope="scope">
            <span class="text-primary">{{ scope.row.msg }}</span>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    async option() {
      ...
      const res = await this.$api.admin.oplogDel({
        ids
      })
      if (res) {
