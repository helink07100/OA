<template>
  <LayoutPage :title="$t('我的审批')">
    ...
    >
      <el-form slot="search" :inline="true" :model="form">
        ...
        <el-form-item>
          <el-select
            ...
            :style="{ width: '140px' }"
          >
            <el-option :label="$t('审批状态')" value="0"></el-option>
            <el-option
              v-for="(item, i) in formTypes"
              :key="i"
              :label="item.cn_name"
        ...
        <el-form-item label="">
          <el-select
            v-model="form.is_handel"
            value-key="id"
            :filterable="true"
            :placeholder="$t('状态')"
            :style="{ width: '100px' }"
          >
            <el-option :label="$t('处理状态')" value=""></el-option>
            <el-option
              v-for="item in handleTypes"
              :key="item.name"
              :label="item.name"
              :value="item.id"
            ></el-option>
      ...
      >
        <el-table-column prop="create_time" :label="$t('申请日期')" width="120">
        ...
        <el-table-column prop="form_name" :label="$t('标题')">
          <template slot-scope="scope">
            <a
              ...
              class="text-primary"
              :href="
                '/myflow/record_detailed?title=查看审批&record_id=' +
                  scope.row.record_id
              "
        ...
        <el-table-column :label="$t('申请人')">
          <template slot-scope="scope">
            {{ scope.row.department_name + '-' + scope.row.show_name }}
          </template>
        </el-table-column>
        <el-table-column prop="is_handel" :label="$t('处理状态')">
          <template slot-scope="scope">
            <label-status
              v-if="scope.row.is_handel == 1"
              status="warning"
              :name="scope.row.is_handel_text"
            />
            <label-status
              v-else
              status="success"
...
export default {
  ...
  methods: {
    beforeGetList() {
      ...
      return {
        handler: this.$api.myflow.getApproveList,
        params: this.form
      }
    },
