...
// 大文件分块上传
async function splitUpload(file, onProgress, action = '') {
  ...
  const config = {
    ...
    onUploadProgress: (e) => {
      if (param.chunked) {
        ...
        const sum = UPLOAD_STATUS.reduce((a, c) => a + c)
...
function request(options) {
  OPTIONS = options
  UPLOAD_STATUS = []
  const { maxSize, multiUploadSize } = SETTING
  ...
  const rs = uploadFunc(file, onProgress, options.action)
    .then((rs) => {
      const result = rs.length ? rs.find((item) => item.data.realname) : rs
      onSuccess(result)
    })
    .catch((e) => {
      if (e.message) {
