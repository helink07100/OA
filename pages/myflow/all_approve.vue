  ...
  <LayoutPage :title="$t('全部审批')">
    ...
    >
      ...
      >
        ...
        <el-table-column prop="sn" :label="$t('编号')" width="280">
        </el-table-column>
            ...
            <nuxt-link
              ...
              :to="
                '/myflow/record_detailed?title=查看审批&record_id=' +
                  scope.row.record_id
              "
              >{{ scope.row.form_name }}</nuxt-link
            >
          </template>
        </el-table-column>
        <el-table-column prop="show_name" :label="$t('申请人')">
        </el-table-column>
        <el-table-column prop="status" :label="$t('审批状态')" width="200">
          <template slot-scope="scope">
            <label-status
...
export default {
  ...
  methods: {
    beforeGetList() {
      this.form.uid = this.$store.state.user.user.uid
      ...
      return {
        handler: this.$api.myflow.getAllApprove,
        params: this.form
      }
    },
